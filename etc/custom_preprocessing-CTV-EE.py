...
def fix_segmentation_labels(seg):
    ...
    array[array == 2] = 1  # Make 2 CTV to be 1.
...
def preprocess(input_dir, output_dir):
    ...
    for subdir_1 in os.listdir(os.path.join(input_dir)):
        if subdir_1 in ["1", "2", "6", "11", "15", "23"]:
            # These cases have an alignment issue - remove after learning fix.
            continue
        else:
            ...
            seg = fix_segmentation_labels(
                sitk.ReadImage(
                    os.path.join(input_dir, id_) + "_labelmask_E1_final.nii.gz"
                )
            )
            ...
            t1 = sitk.ReadImage(
                os.path.join(input_dir, id_) + "_T1w" + s_add + ".nii.gz"
            )
