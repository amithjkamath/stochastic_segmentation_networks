...
from running_metrics.running_probability_distribution import (
    RunningDistributionStatistics,
)
from running_metrics.samplers import (
    LowRankMultivariateNormalRandomSampler,
    LowRankMultivariateNormalClassWeightedRangeSampler,
    CategoricalSampler,
    CategoricalDeterministicSampler,
)
...
class_names = ["background", "Brainstem", "Hippocampus L", "Hippocampus R"]
...
def get_samplers_deterministic():
    samplers = {
        "random_sampler": {
            "sampler_class": CategoricalSampler,
        ...
        "deterministic_sampler": {
            "sampler_class": CategoricalDeterministicSampler,
            "sampler_kwargs": {"device": torch.device(DEVICE), "seed": None},
            "extra_maps": [],
            "require_prob_maps": True,
            "num_samples": 1,
...
def get_samplers_stochastic():
    samplers = {
        "random_sampler": {
            "sampler_class": LowRankMultivariateNormalRandomSampler,
            "sampler_kwargs": {"device": torch.device(DEVICE), "seed": None},
...
def get_class_weigthed_samplers(scale_range=3):
    ...
    for i in range(4):
        kwargs.update(
            {"class_index": i, "scale_range": np.linspace(-r, r, 2 * r + 1).tolist()}
        )
        samplers.update(
            {
                f"class_weighted_sampler_c_{i:d}": {
...
def evaluate(
    csv_path, deterministic, detailed=False, make_thumbs=False, num_samples_cap=20
):
    ...
    for key, sampler in samplers.items():
        ...
        running_metrics.update(
            {
                key: RunningDistributionStatistics(
                    classes=class_names,
    ...
    for key, sampler in samplers.items():
        if key != "deterministic_sampler" and make_thumbs:
            make_sample_thumbs(
                os.path.join(os.path.dirname(csv_path), key + ".csv"),
                min(sampler["num_samples"], num_samples_cap),
                10,
            )
    ...
    for key in samplers.keys():
        ...
        if (key == "random_sampler" and not deterministic) or (
            deterministic and key == "deterministic_sampler"
        ):
            dist_stats = DistributionStatistics(
                running_metrics[key], class_names, class_mergers
            )
